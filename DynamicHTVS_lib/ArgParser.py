...
class ArgParser:
    def __init__(self):
        from .Utility import GetRightSettings
        GetRightSettings()
        self.argumentParser()
    ...
    def argumentParser():
        ...
        ap.add_argument("-batch", "-batch", required=False, type=int, default=int(len(GPUtil.getGPUs())),
        ...
        ap.add_argument('-e', '--exclude', nargs='*', required=False, action='append',
        ...
        dock, rank, parametrize, build, calculate, selection, amber, launch, batch, exclude, prt = args.dock, args.rank, args.parameterize, args.build, args.calculatescore, args.selection, args.amber, args.launchOpenmm, args.batch, args.exclude, args.prt
        if selection:
            joinedSel = ' '.join(args.selection)
        else:
            joinedSel = None

        return dock, rank, parametrize, build, calculate, joinedSel, amber, launch, batch, exclude, args.consider, prt, args.membraneRestraints, args.boxsize
